	...
	var d = {
		...
		fakeEvent : function(k, i, l, j) {
			...
			switch (m) {
				case "MouseEvents" :
					...
					n.initMouseEvent(i, l.bubbles || false,
							l.cancelable || true, window, l.clickCount || 1, 0,
							0, h, g, false, false, false, false, 0, null);
					break;
				case "KeyboardEvents" :
					...
					o(i, l.bubbles || false, l.cancelable, window, l.ctrlKey,
							l.altKey, l.shiftKey, l.metaKey, l.keyCode,
							l.charCode);
					break;
				default :
		...
		addClass : function(i, h) {
			if (i.className === undefined) {
				...
				if (i.className !== h) {
					...
					if (g.indexOf(h) == -1) {
						g.push(h);
						i.className = g.join(" ").replace(/^\s+/, "").replace(
								/\s+$/, "")
					}
				}
			}
		...
		hasClass : function(h, g) {
			return new RegExp("(?:^|\\s+)" + g + "(?:\\s+|$)")
					.test(h.className) || false
		},
		getWidth : function(h) {
			var g = getComputedStyle(h);
			return c(g["border-left-width"]) + c(g["border-right-width"])
					+ c(g["padding-left"]) + c(g["padding-right"]) + c(g.width)
		},
		getHeight : function(h) {
			var g = getComputedStyle(h);
			return c(g["border-top-width"]) + c(g["border-bottom-width"])
					+ c(g["padding-top"]) + c(g["padding-bottom"])
					+ c(g.height)
		},
	...
	var c = function(e, f, g) {
		...
		if (b.isUndefined(this.__step)) {
			if (this.initialValue == 0) {
				...
				this.__impliedStep = Math.pow(10, Math.floor(Math
...
dat.controllers.NumberControllerBox = (function(d, e, c) {
	...
	c.extend(b.prototype, d.prototype, {
				updateDisplay : function() {
					this.__input.value = this.__truncationSuspended ? this
							.getValue() : a(this.getValue(), this.__precision);
					return b.superclass.prototype.updateDisplay.call(this)
				}
			});
	function a(g, f) {
...
dat.controllers.NumberControllerSlider = (function(d, f, a, b, g) {
	...
	b.extend(c.prototype, d.prototype, {
				updateDisplay : function() {
					var h = (this.getValue() - this.__min)
...
dat.color.toString = (function(a) {
	return function(b) {
		if (b.a == 1 || a.isUndefined(b.a)) {
			...
			return "rgba(" + Math.round(b.r) + "," + Math.round(b.g) + ","
					+ Math.round(b.b) + "," + b.a + ")"
		}
	}
})(dat.utils.common);
		...
		conversions : {
			THREE_CHAR_HEX : {
				read : function(g) {
					...
					return {
						space : "HEX",
						hex : parseInt("0x" + h[1].toString() + h[1].toString()
								+ h[2].toString() + h[2].toString()
								+ h[3].toString() + h[3].toString())
					}
				},
				write : d
			},
...
dat.GUI = dat.gui.GUI = (function(E, G, i, h, f, y, o, r, L, A, t, l, j, c, s) {
	...
	s.extend(g.prototype, {
		...
		addFolder : function(P) {
			if (this.__folders[P] !== undefined) {
				throw new Error('You already have a folder in this GUI by the name "'
						+ P + '"')
			}
			var Q = {
				name : P,
		...
		onResize : function() {
			...
			if (N.__resize_handle) {
				s.defer(function() {
							N.__resize_handle.style.height = N.__ul.offsetHeight
									+ "px"
						})
			}
			if (N.__closeButton) {
	...
	function m(P, N, O) {
		...
		if (O instanceof L) {
			...
			s.each(["updateDisplay", "onChange", "onFinishChange"],
					function(U) {
						var S = O[U];
						var T = R[U];
						O[U] = R[U] = function() {
			...
			O.domElement.insertBefore(R.domElement,
					O.domElement.firstElementChild)
		} else {
			if (O instanceof r) {
				var Q = function(S) {
					...
					if (O instanceof o) {
						...
						if (O instanceof t) {
							...
							O.updateDisplay = s.compose(function(S) {
										N.style.borderLeftColor = O.__color
												.toString();
										return S
									}, O.updateDisplay);
							O.updateDisplay()
	...
	function v(Q) {
		...
		c.bind(P, "click", function() {
					W.innerHTML = JSON.stringify(Q.getSaveObject(), undefined,
							2);
					w.show();
					W.focus();
					W.select()
	...
	function K(P) {
		...
		P.domElement.insertBefore(P.__resize_handle,
				P.domElement.firstElementChild);
		function N(S) {
			S.preventDefault();
			O = S.clientX;
	...
	return g
		...
		dat.controllers.factory = (function(e, c, d, a, g, f, b) {
			return function(i, j) {
				...
				dat.controllers.StringController = (function(c, d, b) {
					var a = function(f, h) {
					...
					b.extend(a.prototype, c.prototype, {
								updateDisplay : function() {
									...
									return a.superclass.prototype.updateDisplay
											.call(this)
								}
							});
					return a
				...
				dat.controllers.BooleanController, dat.utils.common),
		...
		dat.controllers.NumberControllerSlider,
		dat.controllers.OptionController,
		dat.controllers.ColorController = (function(c, d, a, b, f) {
			var e = function(m, r) {
				...
				d.bind(this.__selector, "mousedown", function(t) {
							d.addClass(this, "drag").bind(window, "mouseup",
									function(u) {
				...
				function l(y) {
					...
					var u = 1 - (y.clientY - z.top + document.body.scrollTop)
							/ t;
					if (u > 1) {
						u = 1
					} else {
				...
				function o(v) {
					...
					var t = 1 - (v.clientY - w.top + document.body.scrollTop)
							/ u;
					if (t > 1) {
						t = 1
					} else {
				...
				updateDisplay : function() {
					...
					if (k !== false) {
						...
						f.each(a.COMPONENTS, function(n) {
									if (!f.isUndefined(k[n])
											&& !f
													.isUndefined(this.__color.__state[n])
											&& k[n] !== this.__color.__state[n]) {
										j = true;
										return {}
									}
								}, this);
					...
					this.__temp.a = 1;
					var m = (this.__color.v < 0.5 || this.__color.s > 0.5)
							? 255
							: 0;
					var l = 255 - m;
					f.extend(this.__field_knob.style, {
								marginLeft : 100 * this.__color.s - 7 + "px",
								marginTop : 100 * (1 - this.__color.v) - 7
										+ "px",
								backgroundColor : this.__temp.toString(),
								border : this.__field_knob_border + "rgb(" + m
										+ "," + m + "," + m + ")"
							});
					this.__hue_knob.style.marginTop = (1 - this.__color.h / 360)
							* 100 + "px";
					this.__temp.s = 1;
					this.__temp.v = 1;
					g(this.__saturation_field, "left", "#fff", this.__temp
									.toString());
					f.extend(this.__input.style, {
								backgroundColor : this.__input.value = this.__color
										.toString(),
								color : "rgb(" + m + "," + m + "," + m + ")",
								textShadow : this.__input_textShadow + "rgba("
										+ l + "," + l + "," + l + ",.7)"
							})
				}
			});
			...
			return e
				...
				dat.controllers.Controller,
				dat.dom.dom,
				dat.color.Color = (function(b, h, c, g) {
					var a = function() {
					...
					Object.defineProperty(a.prototype, "hex", {
								get : function() {
									if (!this.__state.space !== "HEX") {
										this.__state.hex = h.rgb_to_hex(this.r,
												this.g, this.b)
									}
					...
					function e(j, l, k) {
						if (j.__state.space === "HEX") {
							j.__state[l] = h.component_from_hex(j.__state.hex,
									k)
						} else {
							if (j.__state.space === "HSV") {
								g.extend(j.__state, h.hsv_to_rgb(j.__state.h,
												j.__state.s, j.__state.v))
							} else {
					...
					return {
						...
						rgb_to_hsv : function(k, j, d) {
							var e = Math.min(k, j, d), c = Math.max(k, j, d), l = c
									- e, i, f;
							if (c != 0) {
								f = l / c
							} else {
				...
				dat.color.interpret, dat.utils.common),
		dat.utils.requestAnimationFrame = (function() {
			...
			b.prototype.layout = function() {
				this.domElement.style.left = window.innerWidth / 2
						- d.getWidth(this.domElement) / 2 + "px";
				this.domElement.style.top = window.innerHeight / 2
						- d.getHeight(this.domElement) / 2 + "px"
			};
			function a(f) {
				console.log(f)
			}
