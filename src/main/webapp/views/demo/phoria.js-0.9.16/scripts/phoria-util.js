 ...
 *               Polar/planer coordinate conversions and and polygon/line
 *               intersection methods - contribution from Ruan Moolman.
 * @author Kevin Roast
 * @date 10th April 2013
 */

// init glMatrix library - many small Arrays are faster without the use of
// Float32Array wrap/conversion
...
/**
 * Creates a rotation matrix from the given yaw (heading), pitch (elevation) and
 * roll (bank) Euler angles.
 * 
 * @param {mat4}
 ...
 * @param {Number}
 *            yaw the yaw/heading angle in radians
 * @param {Number}
 *            pitch the pitch/elevation angle in radians
 * @param {Number}
 *            roll the roll/bank angle in radians
 * @returns {mat4} out
 */
...
(function() {
	...
	/**
	 * Augment an existing object prototype with additional properties and
	 * functions from another prototype.
	 * 
	 * @param {Object}
	 *            r Receiving object
	 * @param {Object}
	...
	/**
	 * Merge two objects into a new object - does not affect either of the
	 * original objects. Useful for Entity config default and user config
	 * merging. Deep merge returning a combined object. The source overwrites
	 * the target if names match. Nested Arrays contents including objects are
	 * also merged, source values for base datatypes win.
	 */
	...
	/**
	 * Deep combine a source object properties into a target object. Like the
	 * merge function above, this will deep combine object and Arrays and the
	 * contents, however it will overwrite the properties of the target when
	 * doing so.
	 */
	...
	/**
	 * Shallow and cheap (1 level deep only) clone for simple property based
	 * objects. Properties are only safely copied if they are base datatypes or
	 * an array of such. Should only be used for simple structures such as
	...
	/**
	 * Return true if the given mat4 is an identity (noop) matrix, false
	 * otherwise
	 */
	...
	/**
	 * Return a vec3 representing the average world coordinate for the given
	 * polygon vertices
	 */
	...
	/**
	 * Return an Array of a given length using the given factory function to
	 * populate each item
	 */
	...
	/**
	 * Sort a list of polygons by the Z coordinates in the supplied coordinate
	 * list
	 */
	...
	/**
	 * Sort a list of edges by the average Z coordinate of the two vertices that
	 * represent it each edge.
	 */
	...
	/**
	 * Sort a list of points by the Z coordinate. A second list is supplied that
	 * will be sorted in lock-step with the first list (to maintain screen and
	 * worldcoordinate list)
	 */
	Phoria.Util.sortPoints = function sortPoints(coords, worldcoords) {
		// We need our own sort routine as we need to swap items within two
		// lists during the sorting, as
		// they must be maintained in lock-step or the lighting processing
		// (using matching worldcoord indexes)
	 ...
	 * @param level
	 *            TODO: Subdivision level, 0-2 (quads, 2 tris, 4 tris)
	 * @param scale
	 *            Scale of the plane - 1.0 is a unit plane centred on the origin
	 */
	...
	Phoria.Util.generateIcosahedron = function generateIcosahedron(scale) {
		// Generator code from "Tessellation of sphere"
	...
	/**
	 * Subdivide the given vertices and triangles - using a basic normalised
	 * triangle subdivision algorithm. From OpenGL tutorial chapter "Subdividing
	 * to Improve a Polygonal Approximation to a Surface". NOTE: this only works
	 * on triangles or quads not higher order polygons.
	 * 
	 * TODO: currently this subdivide does not reuse vertices that are shared by
	 * polygons!
	 */
	...
	/**
	 * Generate the geometry for a sphere - triangles form the top and bottom
	 * segments, quads form the strips.
	 */
	...
	/**
	 * Generate an Image for a radial gradient, with the given inner and outer
	 * colour stops. Useful to generate quick sprite images of blurred spheres
	 * for explosions, particles etc.
	 */
	...
	/**
	 * Make an XHR request for a resource. E.g. for loading a 3D object file
	 * format or similar.
	 * 
	 * @param config
	 *            JavaScript object describing the url, method, callback and so
	 *            on for the request: { url: url // url of resource (mandatory)
	 *            method: "GET" // HTTP method - default is GET
	 *            overrideMimeType: mimetype // optional mimetype override for
	 *            response stream requestContentType: mimetype // optional
	 *            request Accept content-type fnSuccess: function // success
	 *            handler function - function(responseText, responseJSON)
	 *            fnFailure: function // failure handler function -
	 *            function(responseText, responseJSON) data: string // data for
	 *            POST or PUT method }
	 */
	...
	/**
	 * Geometry importer for Wavefront (.obj) text 3D file format. The url is
	 * loaded via an XHR request and a callback function is executed on
	 * completion of the import and processing.
	 * 
	 * @param config
	 *            JavaScript object describing the url and configuration params
	 *            for the import: { url: url // url of resource (mandatory)
	 *            fnSuccess: function // callback function to execute once
	 *            object is loaded - function({points:[], polygons:[]})
	 *            fnFailure: function // optional callback function to execute
	 *            if an error occurs scale: 1.0 // optional scaling factor - 1.0
	 *            is the default scaleTo: 1.0 // optional automatically scale
	 *            object to a specific size center: false // optional centering
	 *            of imported geometry to the origin reorder: false // true to
	 *            switch order of poly vertices if back-to-front ordering }
	 */
	Phoria.Util.importGeometryWavefront = function importGeometryWavefront(
		...
		Phoria.Util.request({
			...
			fnSuccess : function(data) {
				...
				if (config.center) {
					// calculate centre displacement for object and adjust each
					// point
					var cdispx = (minx + maxx) / 2.0, cdispy = (miny + maxy)
							/ 2.0, cdispz = (minz + maxz) / 2.0;
				...
				if (config.scaleTo) {
					// calc total size multipliers using max object limits and
					// scale
	...
	Phoria.Util.calculatePolarFromPlanar = function calculatePolarFromPlanar(
		...
		// t is theta and represents the vertical angle from the z axis to the
		// point
		point[1] = Math.acos(planar[2] / point[0]);
		// p is phi and represents the horizontal angle from the x axis to the
		// point
		if (planar[0] !== 0) {
			if (planar[0] > 0)
	...
	Phoria.Util.calculatePlanarFromPolar = function calculatePlanarFromPolar(
		...
		return new vec3.fromValues(
				...
				Math.round(polar[0] * Math.sin(polar[1]) * Math.cos(polar[2])
						* 100)
						/ 100,
				// calculate y value from polar coordinates
				Math.round(polar[0] * Math.sin(polar[1]) * Math.sin(polar[2])
						* 100)
						/ 100,
	...
	Phoria.Util.planeLineIntersection = function planeLineIntersection(
		...
		// giving: lineScalar = planeNormal . (planePoint - linePoint) /
		// planeNormal . lineVector
		...
		if (dotProduct !== 0) {
			...
			// return null if parallel, as the vector will never intersect the
			// plane
			return null;
		}
	}
		...
		for (var l = 0; l < polygon.vertices.length; l++) {
			...
			// drop y coordinates
				...
				if (l < polygon.vertices.length - 1)
					...
					point2 = vec2.fromValues(points[polygon.vertices[0]][2],
			...
			// drop x coordinates
				...
				if (l < polygon.vertices.length - 1)
					...
					point2 = vec2.fromValues(points[polygon.vertices[0]][1],
			...
			// check if the vector from the intersection point intersects the
			// line section
