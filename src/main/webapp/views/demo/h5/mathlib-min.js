...
function calcNormalVector(b, d, f, a, c, e) {
	return new Vector3D((d * e) - (f * c), -((e * b) - (a * f)), (b * c)
					- (d * a)).norm()
}
...
(function() {
	...
	Vector.prototype = {
		...
		distance : function(v) {
			var xx = this.x - v.x, yy = this.y - v.y;
		...
		thetaTo : function(vec) {
			var v = this.clone().norm(), w = vec.clone().norm();
		...
		nrotate : function(a) {
			var ca = Cos(a), sa = Sin(a);
			return new Vector(this.x * ca - this.y * sa, this.x * sa + this.y
							* ca)
		},
		invert : function() {
			this.x = -this.x;
...
(function() {
	...
	Vector3D.prototype = {
		...
		cross : function(a) {
			return new Vector3D(this.y * a.z - this.z * a.y, this.z * a.x
		...
		distance : function(b) {
			var c = this.x - b.x, d = this.y - b.y, a = this.z - b.z;
			return Sqrt(c * c + d * d + a * a)
		},
		thetaTo : function(b) {
			var a = this.y * b.z - this.z * b.y, d = this.z * b.x - this.x
					* b.z, c = this.x * b.y - this.y * b.x;
