...
(function() {
	...
	K3D.BaseController.prototype = {
		...
		resetBackground : function(j) {
			if (this.clearBackground) {
				if (this.fillStyle) {
					j.fillStyle = this.fillStyle
				}
				switch (this.clearingStrategy) {
					case "all" :
						if (this.fillStyle) {
							j.fillRect(0, 0, this.canvas.width,
									this.canvas.height)
						} else {
							j.clearRect(0, 0, this.canvas.width,
									this.canvas.height)
						}
					case "eachobject" :
						for (var d = 0, e = this.objects.length, c, b; d < e; d++) {
							c = this.objects[d];
							switch (c.drawmode) {
								case "point" :
									b = (c.outputlinescale
									...
									if (this.fillStyle) {
										j
												.fillRect(
														Floor(c.rminx) - b,
														Floor(c.rminy) - b,
														(Ceil(c.rmaxx) - Floor(c.rminx))
																+ b * 2 + 1,
														(Ceil(c.rmaxy) - Floor(c.rminy))
																+ b * 2 + 1)
									} else {
										j
												.clearRect(
					...
					case "combineobjects" :
						var a = 1, k = 4096, g = -4096, h = 4096, f = -4096;
						for (var d = 0, e = this.objects.length, c, b; d < e; d++) {
							c = this.objects[d];
							switch (c.drawmode) {
								case "point" :
						...
						if (this.fillStyle) {
							j.fillRect(Floor(k) - a, Floor(h) - a,
									(Ceil(g) - Floor(k)) + a * 2 + 1,
									(Ceil(f) - Floor(h)) + a * 2 + 1)
						} else {
							j.clearRect(Floor(k) - a, Floor(h) - a,
									(Ceil(g) - Floor(k)) + a * 2 + 1,
									(Ceil(f) - Floor(h)) + a * 2 + 1)
						}
				}
...
(function() {
	...
	extend(K3D.Controller, K3D.BaseController, {
				...
				addK3DObject : function(a) {
					a
							.setController(this, this.canvas.width,
									this.canvas.height);
					this.objects.push(a)
				},
				tick : function() {
...
(function() {
	K3D.RequestAnimController = function(b, a) {
		K3D.Controller.superclass.constructor.call(this);
		this.canvas = b;
		this.framecount = 1;
		var c = this;
	...
	extend(K3D.RequestAnimController, K3D.BaseController, {
				...
				frame : function() {
					...
					this.resetBackground(b);
					this.processFrame(b);
					var f = (Date.now() - g);
					if (!this.paused) {
						var a = ~~(1000 / 60 - f), e = this;
					...
					if (DEBUG && DEBUG.FPS) {
						b.save();
						var d = Math.round(1000 / (g - this.lastFrameStart)), c = Math
								.round((this.totalfps += d)
										/ (this.framecount++));
						b.fillStyle = "white";
						b.fillRect(4, 4, 128, 10);
						b.fillStyle = "black";
						b.fillText("TPF: " + f + " FPS: " + d + " AVF: " + c,
								4, 12);
						b.restore()
					}
...
(function() {
	...
	K3D.BaseObject.prototype = {
		...
		doublesided : false,
		calcMatrix : function() {
			var b = this.angles, a = this.matrix;
			b[0] = Sin(this.ogamma * RAD);
			b[1] = Cos(this.ogamma * RAD);
			b[2] = Sin(this.otheta * RAD);
			b[3] = Cos(this.otheta * RAD);
			b[4] = Sin(this.ophi * RAD);
			b[5] = Cos(this.ophi * RAD);
			a[0][0] = b[5] * b[1];
			a[1][0] = -(b[5] * b[0]);
			a[2][0] = b[4];
	...
	extend(K3D.K3DObject, K3D.BaseObject, {
		...
		init : function(v, l, f) {
			...
			for (var q = 0, a, o, t; q < 256; q++) {
				...
				this.depthcueColors[q] = "rgb(" + Ceil(a) + "," + Ceil(o) + ","
						+ Ceil(t) + ")"
			}
			for (var n = 0, k = f.length, p, e, u, m, d, s, h; n < k; n++) {
				p = f[n].vertices;
				e = v[p[1]].x - v[p[0]].x;
				u = v[p[1]].y - v[p[0]].y;
				m = v[p[1]].z - v[p[0]].z;
		...
		transformToWorld : function() {
			var k, h, f;
			var p = this.points, e = this.worldcoords, b = this.faces, n = this.matrix;
			var l = this.aboutx, j = this.abouty, g = this.aboutz, d = this.offx, c = this.offy, a = this.offz;
			var r = n[0], o = n[1], m = n[2];
			...
			for (var q = 0, s = b.length, t; q < s; q++) {
				...
				h = t.y;
				f = t.z;
				b[q].worldnormal = new Vector3D((r[0] * k) + (r[1] * h)
								+ (r[2] * f), (o[0] * k) + (o[1] * h)
								+ (o[2] * f), (m[0] * k) + (m[1] * h)
								+ (m[2] * f))
			}
		...
		transformToScreen : function() {
			var k, g, e, a = this.worldcoords, h = this.screencoords, m = this.screenx, l = this.screeny, d = this.perslevel;
			this.rminx = this.rminy = 4096;
			this.rmaxx = this.rmaxy = -4096;
			for (var b = 0, j, f, c = this.points.length; b < c; b++) {
				k = a[b].x;
				g = a[b].y;
				e = a[b].z + d;
				if (e === 0) {
		...
		executePipeline : function() {
			if (this.recalculateNormals) {
				var c = this.faces, m = this.points;
				for (var g = 0, e = c.length, h, b, l, f, a, k, d; g < e; g++) {
					h = c[g].vertices;
					b = m[h[1]].x - m[h[0]].x;
					l = m[h[1]].y - m[h[0]].y;
					f = m[h[1]].z - m[h[0]].z;
	...
	extend(K3D.LightSource, K3D.BaseObject, {
				...
				transformToWorld : function() {
					...
					this.worldvector = new Vector3D(
							(b[0][0] * a) + (b[0][1] * d) + (b[0][2] * c)
									+ this.offx, (b[1][0] * a) + (b[1][1] * d)
									+ (b[1][2] * c) + this.offy, (b[2][0] * a)
									+ (b[2][1] * d) + (b[2][2] * c) + this.offz)
				}
			})
})();
(function() {
	...
	extend(K3D.PointRenderer, K3D.Renderer, {
				sortByDistance : function(a) {
					if (a.shademode !== "plain" && a.sortmode === "sorted") {
						this.quickSortObject(a.screencoords, a.worldcoords, 0,
								a.points.length - 1)
					}
				},
				...
				renderObject : function(f, m) {
					var b, h, k;
					var j = f.screencoords, d = f.worldcoords, n = f.depthscale, a = n
							/ 128, l = f.linescale / 255;
...
(function() {
	...
	extend(K3D.WireframeRenderer, K3D.Renderer, {
		sortByDistance : function(a) {
			if (a.shademode !== "plain" && a.sortmode === "sorted") {
				this.quickSortObject(a.worldcoords, a.edges, 0, a.edges.length
								- 1)
			}
		},
		...
		renderObject : function(h, q) {
			var k, n, m, o;
			var f = h.edges, l = h.screencoords, e = h.worldcoords;
			var r = h.depthscale, d = r / 128, p = h.linescale / 255;
			q.lineWidth = h.linescale;
...
(function() {
	...
	extend(K3D.SolidRenderer, K3D.Renderer, {
		sortByDistance : function b(g) {
			if (g.sortmode === "sorted") {
				this.quickSortObject(g.worldcoords, g.faces, 0, g.faces.length
								- 1)
			}
		},
		quickSortObject : function a(j, g, i, h) {
			var l, k;
			g.sort(function(m, q) {
						l = m.vertices;
						for (var n = 0, p = 0; n < l.length; n++) {
							p += j[l[n]].z
						}
						p = p / l.length;
						k = q.vertices;
						for (var n = 0, o = 0; n < k.length; n++) {
							o += j[k[n]].z
						}
						o = o / k.length;
						return (p < o ? 1 : -1)
		...
		renderObject : function c(y, D) {
			var l = y.faces, m = y.screencoords, s = y.worldcoords;
			var p = y.depthscale, L = p / 128;
			var z = new Vector3D(0, 0, 1);
			var u, A, H, K, J, x = PI / 2, t, N = y.controller.lights;
			var M = y.doublesided;
			var C = (1 / y.perslevel) * 256;
			if (y.fillmode === "fillstroke") {
				D.lineWidth = 1
			}
			for (var B = 0, G = l.length, w; B < G; B++) {
				w = l[B];
				u = w.vertices;
				if (M || z.dot(w.worldnormal) < C) {
					switch (y.shademode) {
						case "plain" :
							if (w.texture === null) {
								J = w.color;
								t = "rgb(" + J[0] + "," + J[1] + "," + J[2]
										+ ")";
								this.renderPolygon(D, y, w, t)
							} else {
								this.renderPolygon(D, y, w)
							}
							break;
						case "depthcue" :
							for (var F = 0, E = u.length, v = 0; F < E; F++) {
								v += s[u[F]].z
							}
							var q = ((v / u.length) + p) / L;
							if (q < 0) {
								q = 0
							} else {
								if (q > 255) {
							...
							if (w.texture === null) {
								...
								q = 255 - Ceil(q);
								t = "rgba(0,0,0," + (1 - (q / 255)) + ")"
							}
							this.renderPolygon(D, y, w, t);
							break;
						case "lightsource" :
							if (N.length === 0) {
								var I = z.thetaTo2(w.worldnormal);
								J = w.color;
								A = Ceil(I * (J[0] / PI));
								H = Ceil(I * (J[1] / PI));
								K = Ceil(I * (J[2] / PI));
								if (w.texture === null) {
									...
									t = "rgba(0,0,0," + (1 - I * ONEOPI) + ")"
								}
								this.renderPolygon(D, y, w, t)
							} else {
								A = H = K = 0;
								for (var F = 0, E = N.length, o, h; F < E; F++) {
									o = N[F];
									I = PI
											- o.worldvector
													.thetaTo2(w.worldnormal);
									h = I
											* ((1 / o.worldvector
													.distance(w.worldnormal)) * o.intensity)
											/ PI;
									A += (h * o.color[0]);
									H += (h * o.color[1]);
									K += (h * o.color[2])
		...
		renderPolygon : function d(t, l, o, s) {
			var p = l.screencoords, n = o.vertices;
			t.save();
			if (o.texture === null) {
				if (l.fillmode === "inflate") {
					var g = this.inflatePolygon(n, p);
					t.beginPath();
					t.moveTo(g[0].x, g[0].y);
					for (var m = 1, h = n.length; m < h; m++) {
						t.lineTo(g[m].x, g[m].y)
					}
					t.closePath()
				} else {
					t.beginPath();
					t.moveTo(p[n[0]].x, p[n[0]].y);
					for (var m = 1, h = n.length; m < h; m++) {
						t.lineTo(p[n[m]].x, p[n[m]].y)
					}
					t.closePath()
				}
				switch (l.fillmode) {
				...
				var q = l.textures[o.texture];
				var k = function(G, D, L, C, J, A, H) {
					var B = this.inflatePolygon(G, p);
					t.save();
					t.beginPath();
					t.moveTo(B[0].x, B[0].y);
					for (var K = 1, I = G.length; K < I; K++) {
						t.lineTo(B[K].x, B[K].y)
					}
					t.closePath();
					t.clip();
					var O = B[0].x, x = B[0].y, N = B[1].x, w = B[1].y, M = B[2].x, v = B[2].y;
					var u = u = 1 / (D * (H - J) - C * H + A * J + (C - A) * L);
					var Q = -(L * (M - N) - J * M + H * N + (J - H) * O) * u, P = (J
							* v + L * (w - v) - H * w + (H - J) * x)
							* u, z = (D * (M - N) - C * M + A * N + (C - A) * O)
							* u, y = -(C * v + D * (w - v) - A * w + (A - C)
							* x)
							* u, F = (D * (H * N - J * M) + L * (C * M - A * N) + (A
							* J - C * H)
							* O)
							* u, E = (D * (H * w - J * v) + L * (C * v - A * w) + (A
							* J - C * H)
							* x)
							* u;
					t.transform(Q, P, z, y, F, E);
					t.drawImage(q, 0, 0);
					if (s) {
						t.fillStyle = s;
						t.fill()
				...
				k
						.call(this, n.slice(0, 3), 0, 0, q.width, 0, q.width,
								q.height);
				if (n.length === 4) {
					var r = [];
					r.push(n[2]);
					r.push(n[3]);
					r.push(n[0]);
					k.call(this, r, q.width, q.height, 0, q.height, 0, 0)
				}
			}
			t.restore()
		},
		inflatePolygon : function e(p, r) {
			var s = [], l = new Array(p.length);
			for (var n = 0, m = p.length, h, t, g, q, v, u, o; n < m; n++) {
				h = r[p[n]].x;
				t = r[p[n]].y;
				if (n < m - 1) {
					g = r[p[n + 1]].x;
			...
			for (var n = 0, m = p.length, k; n < m; n++) {
				if (n === 0) {
					k = this.intersection(s[(m - 1) * 2], s[(m - 1) * 2 + 1],
							s[0], s[1])
				} else {
					k = this.intersection(s[(n - 1) * 2], s[(n - 1) * 2 + 1],
							s[n * 2], s[n * 2 + 1])
				}
				if (Abs(k.x - r[p[n]].x) > 1.5 || Abs(k.y - r[p[n]].y) > 1.5) {
					k.x = r[p[n]].x;
					k.y = r[p[n]].y
				}
				l[n] = k
		...
		intersection : function f(j, i, n, m) {
			var h, p, l, g, o, k, q;
			h = i.x - j.x;
			p = n.x - m.x;
			l = n.x - j.x;
